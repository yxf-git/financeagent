export const getAssetsFiles = (url: string) => {
  return new URL(`../assets/img/item/${url}`, import.meta.url).href;
};
...
export class DocTypewriter {
  ...
  add(str: string) {
    if (!str) return;
    if (str === "[end]") {
      this.queue = this.queue + "骉"
      return
    }
    this.queue = this.queue + str;
  }
  // 消费
  consume() {
    if (this.queue.length > 0) {
      const str = this.queue.slice(0, 1);
